...
if [ ! -f "$SIDEBAR_FILE" ]; then
    ...
    if grep -q "PROTEKSI_X4NDO12 PARADOX_APPAPI_MENU" "$SIDEBAR_FILE"; then
        ...
        if [ -n "$LINE_NUM" ]; then
            ...
            sed -i "${END_LINE}a\\{{-- END PROTEKSI_X4NDO12 PARADOX_APPAPI_MENU --}}" "$SIDEBAR_FILE"
            sed -i "${END_LINE}a\\@endif" "$SIDEBAR_FILE"

            # Insert @if sebelum START_LINE
            sed -i "$((START_LINE))i\\@if(Auth::user()->id === 1)" "$SIDEBAR_FILE"
            sed -i "$((START_LINE))i\\{{-- PROTEKSI_X4NDO12 PARADOX_APPAPI_MENU: Sembunyikan untuk non-ID 1 --}}" "$SIDEBAR_FILE"
...
if [ ! -f "$API_CONTROLLER" ]; then
    ...
    if grep -q "PROTEKSI_X4NDO12 PARADOX_APPAPI_BLOCK" "$API_CONTROLLER"; then
        echo "⚠️ Proteksi sudah ada, skip..."
    else
        ...
        if [ -n "$INDEX_LINE" ]; then
            ...
            sed -i "${BRACE_LINE}a\\        // PROTEKSI_X4NDO12 PARADOX_APPAPI_BLOCK: Block akses untuk non-ID 1" "$API_CONTROLLER"
        ...
        if [ -n "$STORE_LINE" ]; then
            ...
            sed -i "${BRACE_LINE}a\\        // PROTEKSI_X4NDO12 PARADOX_APPAPI_BLOCK" "$API_CONTROLLER"
            sed -i "$((BRACE_LINE + 1))a\\        if (\\\\Auth::user()->id !== 1) { abort(403, 'Akses Application API tidak diizinkan.'); }" "$API_CONTROLLER"
